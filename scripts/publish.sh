...
# confirme par email. N'est appelé que par scripts/check_validation.py, lui
# -même déclenché uniquement après une réponse email affirmative de l'auteur.
#
...
notify() { "$PYTHON_BIN" "$SCRIPTS_DIR/notify_author.py" "$1" "$2" >>"$RUN_LOG" 2>&1 || log "échec de l'envoi email (sujet : $1)"; }
...
if ! git push origin main >>"$RUN_LOG" 2>&1; then
	...
	notify "Prisme : échec de la mise en ligne" "La validation a été reçue mais le push vers GitHub a échoué. Vérifie automation/logs/ sur le Mac mini."
	exit 1
fi
...
entry = f"- **{today}** — Mise en ligne automatique de la publication \"{theme}\" suite à validation par email : {url}\n"
...
notify "Prisme : \"$THEME_TITLE\" est en ligne" "$SITE_URL"
